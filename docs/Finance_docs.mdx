...
| `RequestWithdrawal` / `GetWithdrawalRequests` | Cria e lista `WithdrawalRequest` bloqueando saldo. Opcionalmente exige confirmação de senha do usuário (configurável via `WALLET_REQUIRE_WITHDRAW_PASSWORD`). | `userId`, `amount`, `currency`, `notes?`, `password?` | Entidade(s) de saque; retorna 403 quando a senha é exigida e inválida/ausente. |
