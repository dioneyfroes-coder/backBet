...
| `Deposit` / `Withdraw` | Orquestram Pix charge/payout antes de tocar a carteira | `Deposit`: `userId`, `amount`, `currency`, `description?` – `Withdraw`: `userId`, `amount`, `currency`, `pixKey`, `description?` | `Deposit` retorna `{ wallet, pixCharge, pixConfirmation }`; `Withdraw` retorna `{ wallet, pixPayout }` |
