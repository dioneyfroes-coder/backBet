...
- `Bet` (`domain/entities/Bet.ts`)
	- Campos: `id`, `userId`, `eventId`, `marketId`, `amount: Money`, `odds: Odds`, `status`, `type`, `createdAt`, `resolvedAt?`, `cancellationReason?`.
...
- `Money` (value object compartilhado de `src/core/shared`) valida valores positivos, garante até duas casas decimais e restringe moedas suportadas.
...
- **Casos de uso** (`application/use-cases/*.ts`)
...
## Integração com RiskService

O sistema agora possui um serviço de risco (`RiskService`) que é consultado durante o fluxo de criação de apostas e que gerencia reserva/liberação de exposição por usuário.

- **Onde é chamado:** `BetService.placeBet` chama `riskService.canPlaceBet(...)` antes de debitar a carteira; após a criação bem-sucedida da aposta chama `riskService.registerExposure(...)`. `BetService.cancelBet` e `BetService.resolveBet` chamam `riskService.reduceExposure(...)` quando aplicável.
- **Regras observadas:** `RiskService` aplica limites de velocidade (velocity), limites por evento/market, whitelist/blacklist de usuários e checagens de exposição cumulativa. A fórmula de liability segue: `liability = stake * (odds - 1)` arredondado a 2 casas.
- **Persistência:** o `RiskService` usa um `IRiskRepository` (implementações Mongoose ou In-memory). A fábrica de persistência (`src/infrastructure/persistence/factory.ts`) expõe `createRiskRepository()`.
- **Observability:** rejeições de risco emitem logs estruturados (`risk_reject`) que servem como hooks para alertas e métricas (Prometheus/Email/PM2 WebUI).

Considere ler `docs/risk-integration.mdx` para detalhes de configuração, toggles e exemplos de comportamento.
...
Endpoints definidos em `BetController` e registrados por `betRoutes` sob `/api/v1/bets`.

| Método | Caminho | Autenticação | Rate limit | Observações |
|--------|---------|--------------|------------|-------------|
| `GET` | `/api/v1/bets/event/:eventId` | pública | n/a | Usa `cacheEventOddsMiddleware` + Redis e retorna `BetListResponseDTO` por evento.
| `POST` | `/api/v1/bets` | Bearer (provedor de identidade ou token dev) | `BET_PLACE_RATE_LIMIT_*` | Valida `PlaceBetDTO`, debita carteira e invalida cache do evento.
| `POST` | `/api/v1/bets/:betId/cancel` | Bearer obrigatório | `BET_CANCEL_RATE_LIMIT_*` | `CancelBetDTO` aceita `reason`; devolve fundos se o evento ainda está `SCHEDULED`.
| `GET` | `/api/v1/bets/me` | Bearer obrigatório | n/a | Lista apostas do usuário autenticado usando `BetListResponseDTO`.
...
export const PlaceBetDTO = z.object({
	...
	marketId: z.string(),
	oddId: z.string(),
	amount: z.number().positive().min(0.01), // reais (decimal) — convertido para amountCents internamente
...
- **Segurança**: autenticação via provedor de identidade configurado (middleware correspondente) ou header Bearer fake em dev; erros retornam `AppError` padronizado.
...
- **Snapshots**: `cacheMetricsSnapshot` e `dependencyHealthSnapshot` fornecem visibilidade rápida de Redis/Mongo e são devolvidos pelos endpoints de health; o PM2 WebUI/alertas de e-mail lerão esses dados.
- **Health checks**: `/health`, `/readiness`, `/health/cache` validam Redis/Mongo — críticos para endpoints de aposta; `/metrics` responde `410 Gone` até que `OBS_ENABLE_PROMETHEUS=true` (compatibilidade Prometheus).
...
- `src/core/betting/application/use-cases/__tests__/BetUseCases.test.ts`: garante delegação correta e mapeamento de erros.
- `src/core/betting/domain/repositories/__tests__/EventRepository.test.ts`: cobre operações do repositório em memória.
- Para e2e use `supertest` ou `curl` com os endpoints de `/api/v1/bets`; scripts como `src/scripts/seedInitialData.ts` ajudam a gerar usuários/carteiras antes das chamadas.
...
4. Expandir testes de integração cobrindo cache + rate-limit e e2e com provedores de identidade/JWT reais.
