...
| Risco | `IRiskRepository` | `MongooseRiskRepository.ts` | `schemas/RiskProfileSchema.ts` | Fallback `InMemoryRiskRepository` disponível quando `USE_MONGOOSE_PERSISTENCE=false`. |
