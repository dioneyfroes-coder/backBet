...
| --- | --- | --- | --- | --- |
| Usuários | `IUserRepository` | `src/infrastructure/persistence/mongoose/repositories/MongooseUserRepository.ts` | `schemas/UserSchema.ts` | Índices únicos em `email` e `username`; sincroniza IDs do provedor de identidade quando configurado. |
| Carteiras | `IWalletRepository` | `MongooseWalletRepository.ts` | `schemas/WalletSchema.ts` | Mantém `transactions[]` embutidas; campos de saldo usam `balanceCents`/`lockedBalanceCents` (centavos inteiros); usa projeções para histórico paginado. |
...
| Risco | `IRiskRepository` | `MongooseRiskRepository.ts` | `schemas/RiskProfileSchema.ts` | Campos `exposureCents`/`maxExposureCents` (centavos inteiros). Fallback `InMemoryRiskRepository` disponível quando `USE_MONGOOSE_PERSISTENCE=false`. |
...
**Observação (Risco / IRiskRepository):** este repositório adiciona gerenciamento de exposição por usuário (reservas de liability). Quando `USE_MONGOOSE_PERSISTENCE=false` a fábrica retorna um `InMemoryRiskRepository` que permite executar testes sem Mongo; quando `true` utiliza `MongooseRiskRepository` com `RiskProfileSchema`.

Ao escrever testes para o repositório Mongoose (incluindo risco), lembre-se que muitas consultas usam encadeamento como `.findOne().lean()` — os mocks Jest devem retornar um objeto com `.lean()` que resolve para o documento esperado.
