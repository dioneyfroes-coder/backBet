...
> Nota: o projeto introduziu um contrato `IRiskRepository` para gerenciar exposição/risco por usuário. Quando `USE_MONGOOSE_PERSISTENCE=false` a fábrica usa `InMemoryRiskRepository` como fallback; quando `true` utiliza `MongooseRiskRepository` (com `RiskProfileSchema`). Considere isto ao planejar testes de integração de risco.
...
| `MONGODB_URI` | `USE_MONGOOSE_PERSISTENCE=true` ou `NODE_ENV=production` | `mongodb://backbet:senha@mongodb:27017/backbet?authSource=admin&replicaSet=rs0` | **Sem default** — falha imediata se ausente (não tenta localhost). |
| `MONGODB_DB_NAME` | `USE_MONGOOSE_PERSISTENCE=true` | `backbet` | Default: derivado do database da `MONGODB_URI`. |
...
1. Ajuste `.env`:
   ...
   MONGODB_URI=mongodb://backbet:senha@mongodb:27017/backbet?authSource=admin&replicaSet=rs0
...
> O projeto possui ferramenta própria testada em `docs/BACKUP_RECOVERY.mdx` (`npm run backup:create` / `backup:restore` / `backup:validate` / `backup:drill`). O `mongodump` abaixo é a alternativa manual para ambientes com a binária instalada.
...
3. Integrar backups com o pipeline (ex.: upload diário para S3 + retenção) — ver `docs/BACKUP_RECOVERY.mdx`.
