...
| Configuração dos endpoints `/health`, `/health/cache`, `/readiness` (para correlação) | `src/infrastructure/api/ApiServer.ts` |
| Documentação geral de observabilidade | `docs/OBSERVABILITY.mdx` |
| Transporte para arquivo (`logs/backbet.log`) | `src/shared/logging/fileLogger.ts`, `src/shared/logging/structuredLogger.ts` |
...
| `request_healthcheck` | `info` | Handlers de `/health*` | `endpoint`, `result`, `cacheEnabled`, `observability` |
| `observability_toggle_state` | `info` | Ao inicializar/alterar toggles `OBS_*` | `toggle`, `value`, `source`, `metadata` |
...
- Além de `stdout`, os mesmos logs são gravados em `logs/backbet.log` (configurável com `LOG_FILE_PATH`).
- Habilite/desabilite com `LOG_FILE_ENABLED` (`true` em produção por padrão). Os limites de rotação são `LOG_FILE_MAX_SIZE_MB` (default 20MB) e `LOG_FILE_MAX_FILES` (default 5 arquivos históricos).
- A rotação é local: ao atingir o limite de tamanho, o arquivo atual vira `.1`, `.1` vira `.2` e assim por diante; o arquivo principal volta a começar vazio.
- Para coexistir com pipelines externos (PM2, Loki, CloudWatch), mantenha `stdout` coletado normalmente e use o arquivo apenas para troubleshooting rápido ou ingestão offline.
...
| `warn` | Requisições atendidas, mas com fallback ou degradação (ex.: dependência `skipped`) | Abra issue se o volume aumentar em 5m │ configure alertas no PM2 WebUI/e-mail |
...
# acompanhar arquivo local
tail -f logs/backbet.log | jq .
```
...
3. Os agentes transmitem para o Loki / Elasticsearch definido internamente (os alertas legados de Grafana/Prometheus ficaram apenas como referência histórica no git).
...
## Integração com snapshots e tracing

- Os mesmos `requestId` e `userId` usados nos logs ficam registrados nos snapshots `cacheMetricsSnapshot`, `resilienceMetricsSnapshot` e `dependencyHealthSnapshot`, facilitando correlação durante o consumo via PM2 WebUI.
- Quando um erro dispara `request_error`, o handler atualiza imediatamente os snapshots (ex.: `resilienceMetricsSnapshot.failures`) garantindo consistência entre logs e dados operacionais.
...
- [ ] Documentou novos campos nesta página e, se necessário, na página de observabilidade (`docs/OBSERVABILITY.mdx`)?

## Próximos passos sugeridos

1. Exportar os arquivos `logs/backbet.log*` para um bucket/VSI quando o PM2 realizar rotação prolongada (cold storage).
2. Enviar logs diretamente para o agente OTLP (`otlp-logs`) quando for habilitado na stack Tempo/Loki (ou equivalente).
