...
| `GET /readiness` | Ping em Redis/Mongo, atualiza `backbet_dependency_*`. | Público (proteja via proxy) |
| `GET /health/cache` | Snapshot de `redisClient.getMetrics()`. | Público |
| `GET /metrics` | Metrics Prometheus do `metricsRegistry`. | Público |
...
Proteja `/health`, `/readiness`, `/metrics` e `/health/cache` via firewall ou reverse proxy em produção caso necessário.
