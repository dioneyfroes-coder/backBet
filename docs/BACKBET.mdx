...
- [ ] **API REST** - Implementar controllers e rotas
  ...
  - [ ] Rate limiting (configurado via `RATE_LIMIT_WINDOW_MS`, `RATE_LIMIT_MAX` e `RATE_LIMIT_MESSAGE` em `.env`; `shared/config/appConfig` centraliza a leitura e usa valores altos para dev)
