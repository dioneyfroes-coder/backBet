...
docs/OBSERVABILITY.mdx                           (referência da stack atual de observabilidade)
```
...
2. `loggingMiddleware` registra `request_start` / `request_end` com duração, IP e userId derivado do token de autenticação (JWT) ou provedor configurado.
...
O backend mantém pequenas estruturas em memória para expor o estado atual de cache, resiliência e dependências. Durante a migração para a PM2 WebUI (descrita neste documento), esses snapshots substituem completamente os gauges do Prometheus:
...
- `dependencyHealthSnapshot` (mantido em `ApiServer`) guarda o último status de Redis e Mongo (e de provedores opcionais configurados) sempre que `readiness` roda; cada item registra `status`, `latencyMs` e timestamp.
...
- `USE_MONGOOSE_PERSISTENCE`, `MONGODB_URI`: determinam se o readiness pingará Mongo (e qual instância) para atualizar `dependencyHealthSnapshot`.
...
- `APP_NAME`: nome do produto exibido nos dashboards e logs.
- `SERVICE_NAME`: identidade técnica do serviço; OpenTelemetry usa esse nome quando `OTEL_SERVICE_NAME` não é definido.

## Dashboards e alertas

Os artefatos antigos de Grafana/Prometheus foram arquivados no git apenas para consulta histórica. A nova stack opera em três camadas já habilitadas:
...
## Métricas de negócio e regras de alerta (Fase 23)

Além das métricas HTTP e de dependências, a aplicação agora expõe métricas de negócio no `/metrics` (quando `OBS_ENABLE_PROMETHEUS=true`):

| Métrica | Significado | Onde é incrementada |
|---------|-------------|---------------------|
| `backbet_bets_total` | Apostas criadas com sucesso | `BetService.placeBet` |
| `backbet_bets_rejected_total` | Apostas rejeitadas (RISK_REJECTED / RISK_LIMIT_EXCEEDED) | `BetService.placeBet` |
| `backbet_bets_won_total` / `backbet_bets_lost_total` | Apostas resolvidas como WON / LOST | `BetService.resolveBet` |
| `backbet_deposits_total{type}` | Créditos efetivados na carteira (type = DEPOSIT/BET_WIN/BET_REFUND) | `WalletService.deposit` |
| `backbet_withdrawals_total` | Saques efetivados (WITHDRAWAL_COMPLETED) | `WalletService.withdraw` / `withdrawLocked` |
| `backbet_risk_rejections_total{reason}` | Bloqueios pelas regras de risco | `RiskService.canPlaceBet` e `BetService` (exposure_limit) |
| `backbet_risk_reconciliation_mismatches_total{kind}` | Divergências corrigidas na reconciliação de exposição | `RiskService.recalculateUserExposure/recalculateCounter` |
| `backbet_transaction_failures_total{resource}` | Transações Mongo abortadas por falha de infraestrutura | `MongooseWalletRepository` / `MongooseHouseTreasuryRepository.withTransaction` |
| `backbet_withdrawal_queue_backlog` | Payouts pendentes na fila de saques (gauge) | `withdrawalQueueMetrics` (polling 15s) |

Replays idempotentes (ledger `referenceId` já aplicado) **não** incrementam `deposits_total`/`withdrawals_total`, preservando a semântica de "movimentação real".

As regras de alerta vivem em `deploy/observability/alerting/alerts.yml` (referenciadas pelo `prometheus.yml` via `rule_files`) e cobrem: Mongo/Redis down, falhas de transação, conflitos de optimistic lock, falhas de payout, backlog da fila de saques, divergências de reconciliação (Treasury e exposição de risco), spike de HTTP 5xx, pico de latência (p95 > 2s), spikes de apostas rejeitadas e conflitos de idempotência. Notificação via Alertmanager fica a cargo do deploy (bloco `alerting` no `prometheus.yml`).
