...
:::note Visão geral
O BackBet é projetado para sobreviver a falhas externas (provedores de identidade, Redis, Mongo), picos de autenticação e indisponibilidade parcial. A seguir detalhamos como API, cache, persistência e observabilidade se combinam para evitar downtime e acelerar a recuperação.
:::
...
| Fallback de autenticação | Permite JWT local ou `Bearer <userId>` em dev quando o provedor de identidade está indisponível | `ALLOW_DEV_BEARER_BYPASS=true` |
...
- Spans OTEL permanecem disponíveis e, junto com `dependencyHealthSnapshot`, indicam degradações. Os painéis antigos (Grafana/Prometheus) foram arquivados; seguimos `docs/OBSERVABILITY.mdx`.
...
1. **Retries inteligentes**: adicione libs como `p-retry` aos adaptadores de provedores externos/Redis para backoff exponencial.
2. **Circuit breaker dedicado**: um por integração crítica (provedor de identidade, Redis, Mongo) com limites ajustados a cada SLA.
3. **Resposta degradê**: quando o provedor de identidade falhar, devolva mensagem orientando o usuário (ex.: `418` com instruções) enquanto o monitor dispara alertas automáticos.
