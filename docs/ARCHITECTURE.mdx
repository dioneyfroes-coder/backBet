...
|----------|-----------|-------------------------|----------------------|
| User Core | `src/core/user` | Cadastro, autenticação local, sincronização com provedores de identidade, atualização de perfil e e-mail. | `User`, `Email` VO |
...
| Betting Core | `src/core/betting` | Eventos, odds, colocação/cancelamento/resolução de apostas. | `Bet`, `Event`, `Money`, `Odds` |
...
- **`ApiServer.ts`** monta Express 5 com `helmet`, CORS dinâmico, `express-rate-limit`, parsers, Swagger UI (`/api/docs`) e endpoints de saúde.
- Middlewares próprios: `requestIdMiddleware`, `loggingMiddleware` e handlers de erro padronizados.
- Autenticação:
  - Produção: Passport (`passport-jwt`) ou outro provedor de identidade configurado.
...
- Value Objects encapsulam validações (`Email`, `Money`, `Odds`).
...
- **Autenticação (`src/infrastructure/auth`)**: `JwtService`, adaptadores para provedores de identidade (ex.: Passport) e middlewares complementares.
...
1. **Entrada HTTP** – `ApiServer` aplica middlewares (helmet, CORS, rate-limit, request-id, logging).
2. **Autenticação** – Passport/JWT (ou outro provedor configurado) ou bypass preenche `req.auth.userId`; rotas críticas usam `protectedRoute` para validar token.
...
|---------|-----------|----------------|-------------|
| Usuários | `IUserRepository` | `MongooseUserRepository`, `UserRepository` (in-memory) | Índices únicos em email/username; sincroniza IDs do provedor de identidade quando aplicável. |
...
- **Request Context**: `AsyncLocalStorage` propaga `requestId/userId` para logs, snapshots e handlers de erro.
- **Health/Readiness**: `/health`, `/readiness`, `/health/cache` expostos pelo `ApiServer`. `dependencyHealthSnapshot` sinaliza status e será consumido por PM2 WebUI/alertas.

## Observabilidade

- **Snapshots**: `cacheMetricsSnapshot`, `resilienceMetricsSnapshot` e `dependencyHealthSnapshot` guardam o estado mais recente e são devolvidos via `/health/cache` e `/readiness`. Eles substituem os gauges Prometheus e alimentarão PM2 WebUI/alertas de e-mail.
...
- **Endpoint `/metrics`**: mantido apenas por compatibilidade; retorna `410 Gone` até que `OBS_ENABLE_PROMETHEUS=true` durante transições.
...
→ Cache invalida perfil/carteira e a resposta inclui `status`, `isActive`, snapshot da carteira e (quando o usuário já está ativo) `accessToken`/`refreshToken` + `sessionId`
→ Caso o status permaneça `PENDING_VERIFICATION`, o frontend faz polling usando `GET /api/v1/auth/register/status/:userId` até `isActive=true`
```

### Depósito / Saque
```
POST /api/v1/wallets/deposit | withdraw
→ WalletController verifica rate limit específico e autenticação
→ IdempotencyService reserva Idempotency-Key quando fornecida
...
- `HouseWallet.version` é incrementado a cada mutação persistida; o repositório Mongoose inclui a versão anterior no filtro da atualização e retorna `409 CONFLICT` em caso de conflito.
- `IdempotencyService` usa `SET NX` no Redis para operações financeiras e workers. O resultado concluído pode ser reutilizado; payloads diferentes com a mesma chave são rejeitados.
- O fallback em memória existe apenas para testes/desenvolvimento. Em produção, Redis e Mongo devem estar habilitados e monitorados.
- A transação ACID completa entre carteira, aposta e efeitos externos continua pendente.
...
- Autenticação: Passport/JWT (produção) ou JWT/bypass em dev.
...
4. **Escala Horizontal** – preparar `ApiServer` para múltiplas instâncias atrás de um ingress e externalizar sessões do provedor de identidade.
5. **Observabilidade avançada** – completar integração PM2 WebUI + alertas por e-mail usando os snapshots operacionais.
