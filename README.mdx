...
BackBet é um backend de apostas desportivas construído em **TypeScript/Node 20**, seguindo **DDD**, camadas limpas e observabilidade nativa. Todos os módulos vivem em `src/` e são expostos via Express 5, com autenticação baseada em JWT (ou provedores de identidade configuráveis), cache Redis, persistência Mongo opcional e métricas/tracing prontos para produção.
...
- **Qualidade**: ESLint flat config, Jest + SWC, 372 testes (statements 98.2%, branches 89.5%, functions 98.1%, lines 98.0%), CI local `npm run lint && npm run test --coverage && npm run build`
...
- 372 testes Jest (unidade + integração leve) — statements 98.2%, branches 89.5%, functions 98.1%, lines 98.0%.
