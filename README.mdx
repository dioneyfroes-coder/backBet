...
- [ ] **API REST** - Implementar controllers e rotas
  ...
  - [ ] Rate limiting (variáveis `RATE_LIMIT_*` definidas no `.env` leem-se via `shared/config/appConfig`, mantemos limites altos enquanto estamos em dev)
  - [ ] Endpoints `/auth/register`, `/auth/login` e `/auth/refresh` agora possuem limitadores específicos que usam `AUTH_REGISTER_RATE_LIMIT_*`, `AUTH_LOGIN_RATE_LIMIT_*` e `AUTH_REFRESH_RATE_LIMIT_*` para evitar abusos
  - [ ] `createApiRouter` já compõe `/auth`, `/users`, `/wallets`, `/bets` e `/finance` usando repositórios compartilhados e o novo `ClerkService` quando há uma `CLERK_API_KEY` válida
...
[ ] Atualizar documentação da API (OpenAPI + docs/API_DOCS.mdx) para refletir o roteador compartilhado e o `ClerkService`
