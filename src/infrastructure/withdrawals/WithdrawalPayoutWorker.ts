...
export async function processWithdrawalPayload(
  ...
  try {
    const res = await adapter.payWithdrawal(payload.requestId, payload.userId, payload.amount, payload.currency);
    if (res.success) {
      try {
      ...
      writeStructuredLog({ event: 'withdrawal_payout_success', requestId: payload.requestId, tx: res.transactionId });
    } else {
      try {
      ...
      writeStructuredLog({ event: 'withdrawal_payout_failed', requestId: payload.requestId, error: res.error });
...
export function startWithdrawalWorker(): BullQueue {
  ...
  queue.process('payout', async (job) => {
    return processWithdrawalPayload(job.data as WithdrawalPayoutPayload).then(() => Promise.resolve());
  });

  queue.on('failed', (job, err) => {
    writeStructuredLog({ event: 'withdrawal_payout_job_failed', requestId: (job?.data as any)?.requestId, err });
  });
